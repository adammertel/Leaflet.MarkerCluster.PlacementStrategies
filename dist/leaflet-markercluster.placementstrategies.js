...
/*
  ...
  Adam Mertel | univie
*/
"use strict";/*global L:true*/L.MarkerCluster.include({spiderfy:function a(){if(this._group._spiderfied===this||this._group._inZoomAnimation){return}var b=this._group.options;var c=this.getAllChildMarkers();var d=this._group;var e=d._map;var f=e.latLngToLayerPoint(this._latlng);var g=[];if(!(this._group.getLayers()[0]instanceof L.CircleMarker)){f.y+=10}// add options.spiderfiedClassName to the spiderfied markers
if(b.spiderfiedClassName){for(var h in c){var i=c[h];// marker
if(i.getIcon){var j=i.getIcon();if(j){if(j.options.className){if(!j.options.className.includes(b.spiderfiedClassName)){j.options.className+=" "+b.spiderfiedClassName}}else{j.options.className=b.spiderfiedClassName}}//circleMarker
}else if(c[h].setStyle){var k=c[h].options.className;c[h].setStyle({className:k+" "+b.spiderfiedClassName})}}}this._group._unspiderfy();this._group._spiderfied=this;this._clockHelpingGeometries=[];// TODO Maybe: childMarkers order by distance to center
// applies chosen placement strategy
switch(this._group.options.elementsPlacementStrategy){case"default":if(c.length>=this._circleSpiralSwitchover){g=this._generatePointsSpiral(c.length,f)}else{g=this._generatePointsCircle(c.length,f)}break;case"spiral":g=this._generatePointsSpiral(c.length,f);break;case"one-circle":g=this._generatePointsCircle(c.length,f);break;case"concentric":g=this._generatePointsConcentricCircles(c.length,f);break;case"clock":g=this._generatePointsClocksCircles(c.length,f,false);break;case"clock-concentric":g=this._generatePointsClocksCircles(c.length,f,true);break;case"original-locations":g=this._getOriginalLocations(c,this._group._map);break;default:}this._animationSpiderfy(c,g)},unspiderfy:function a(b){/// <param Name="zoomDetails">Argument from zoomanim if being called in a zoom animation or null otherwise</param>
